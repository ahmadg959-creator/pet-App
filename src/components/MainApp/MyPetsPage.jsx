...
import { calculateAge, callGeminiAPI, calculateAgeAtDate } from '../../utils';
...
const MyPetsPage = () => {
        ...
        const { pets, setPets, showToast } = useOutletContext();
        ...
        useEffect(() => {
            if (petId) {
                ...
                const petExists = pets.some(p => p.id === id);
                if(petExists) {
                    setSelectedPetId(id);
                    setView('pet-detail');
                }
...
const PetDetailView = ({ pet, onUpdatePet, onBack, onOpenReminderModal, showToast }) => {
 ...
 'tab-active' : 'text-slate-500 hover:text-slate-700 border-transparent'}`}>Media Gallery</button>
            ...
            <div id="tab-content-container">
                ...
                {activeTab === 'gallery' && <MediaGallery pet={pet} onUpdatePet={onUpdatePet} />}
...
const VaccinationsSection = ({ pet, onUpdatePet, showToast }) => {
    ...
    const handleSave = (formData) => {
        ...
        const updatedPetWithVaccine = { ...pet, health: { ...pet.health, vaccinations: updatedVaccinations } };
        onUpdatePet(updatedPetWithVaccine);
        setShowForm(false);
        setEditingVaccine(null);

        // Suggest a reminder for any vaccine added without a next due date
        if (!editingVaccine && !formData.nextDue && formData.date) {
            const vaccineDate = new Date(formData.date);
            vaccineDate.setFullYear(vaccineDate.getFullYear() + 1);
            const reminderDate = vaccineDate.toISOString().split('T')[0];

            if (window.confirm(`Would you like to create a reminder for the next ${formData.name} vaccine in one year?`)) {
                const newReminder = {
                    id: Date.now(),
                    title: `${formData.name} Vaccine Booster`,
                    type: 'Vet',
                    date: reminderDate,
                    time: '09:00',
                    repeat: 'None',
                    notes: `Annual booster shot for ${formData.name}, following the vaccination on ${formData.date}.`,
                    status: 'pending'
                };
                const updatedReminders = [...(updatedPetWithVaccine.reminders || []), newReminder];
                onUpdatePet({ ...updatedPetWithVaccine, reminders: updatedReminders });
                showToast(`✅ Reminder for ${formData.name} booster created!`);
            }
        }
    };
    ...
    return (
        <section>
            ...
            {sortedVaccinations.length === 0 && !showForm && (
                <EmptyState
                    icon="💉"
                    message="No vaccination records yet. Keep track of your pet's shots to stay on top of their health."
                    buttonText="Add First Vaccination"
                    onButtonClick={() => { setEditingVaccine(null); setShowForm(true); }}
                />
            )}
        </section>
    );
...
const DewormingSection = ({ pet, onUpdatePet, showToast }) => {
    ...
    const handleSave = (formData) => {
        ...
        const updatedPetWithDeworming = { ...pet, health: { ...pet.health, deworming: updatedDeworming } };
        onUpdatePet(updatedPetWithDeworming);
        setShowForm(false);
        setEditingDeworm(null);

        // Suggest a reminder for deworming if no next due date is provided
        if (!editingDeworm && !formData.nextDue && formData.date) {
            const dewormDate = new Date(formData.date);
            dewormDate.setMonth(dewormDate.getMonth() + 3); // Suggest a 3-month reminder
            const reminderDate = dewormDate.toISOString().split('T')[0];

            if (window.confirm(`Would you like to create a reminder for the next deworming in 3 months?`)) {
                const newReminder = {
                    id: Date.now(),
                    ...
                    time: '09:00',
                    repeat: 'None',
                    notes: `3-month deworming follow-up for ${formData.name}.`,
                    status: 'pending'
                };
                const updatedReminders = [...(updatedPetWithDeworming.reminders || []), newReminder];
                onUpdatePet({ ...updatedPetWithDeworming, reminders: updatedReminders });
                showToast(`✅ Reminder for deworming created!`);
            }
        }
    };
    ...
    return (
        <section>
            ...
            {sortedDeworming.length === 0 && !showForm && (
                <EmptyState
                    icon="🦠"
                    message="No deworming records found. Regular treatment is key to your pet's well-being."
                    buttonText="Add First Deworming Record"
                    onButtonClick={() => { setEditingDeworm(null); setShowForm(true); }}
                />
            )}
        </section>
    );
        ...
        <section>
            ...
            <div id="medication-list" className="space-y-3">
                {(pet.health.medications || []).length > 0 ? (
                    ...
                    !showForm && <EmptyState
                        icon="💊"
                        message="No current medications being tracked."
                        buttonText="Add Medication"
                        onButtonClick={() => { setEditingMed(null); setShowForm(true); }}
                    />
                )}
            </div>
        </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    {sortedHistory.length === 0 && !showHistoryForm && (
                        <EmptyState
                            icon="🏥"
                            message="No past medical events logged. Add an event to build your pet's history."
                            buttonText="Add First Event"
                            onButtonClick={() => { setEditingHistory(null); setShowHistoryForm(true); }}
                        />
                    )}
                </div>
            </div>
